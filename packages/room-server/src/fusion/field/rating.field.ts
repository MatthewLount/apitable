...
export class RatingField extends BaseNumberField implements OnApplicationBootstrap {
  override validate(fieldValue: IFieldValue, field: IField, extra?: { [key: string]: string }) {
    ...
    if (!isNumber(fieldValue) || Number.isNaN(fieldValue)) {
      this.throwException(field, ApiTipConstant.api_param_rating_field_type_error, extra);
    }
    // Determine if the maximum value is exceeded
    if (fieldValue > field.property.max) {
      this.throwException(field, ApiTipConstant.api_params_rating_field_max_error);
    }
    if (fieldValue < 0) {
      this.throwException(field, ApiTipConstant.api_param_invalid_rating_field);
    }
  }
